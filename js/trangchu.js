window.onload = function () {
	...
	$('.owl-carousel').owlCarousel({
		...
		nav: true,          // ✅ Hiện mũi tên điều hướng
	});
	});
	...
	if (filters.length) { // có filter
		...
		//  =============Cột sản phâm màn hình nhỏ thì hiển thị 4 sp, to thì hiển thị 5=================
	...
	// Thêm chọn mức giá
	addPricesRange(0, 5000000);
	addPricesRange(5000000, 10000000);
...
function addBanner(img, link) {
	...
	document.querySelector('.owl-carousel').innerHTML += newDiv;
}

// Thêm hãng sản xuất
function addCompany(img, nameCompany) {
	var link = createLinkFilter('add', 'company', nameCompany);
	var newTag = `<a href="${link}" title="${nameCompany}">
					<img src="${img}" alt="${nameCompany}">
				  </a>`;
	document.querySelector('.companyMenu').innerHTML += newTag;
}

// Thêm chọn mức giá
function addPricesRange(min, max) {
	let text = '';
	if (max === 0) text = `Trên ${min.toLocaleString()}₫`;
	else text = `${min.toLocaleString()}₫ - ${max.toLocaleString()}₫`;

	var link = createLinkFilter('add', 'price', `${min}-${max}`);
	var priceHTML = `<a href="${link}">${text}</a>`;
	document.querySelector('.pricesRangeFilter .dropdown-content').innerHTML += priceHTML;
}
...
function addPromotion(name) {
	...
	var text = promoMap[name] || name;
	var link = createLinkFilter('add', 'promo', name);
	var promoHTML = `<a href="${link}">${text}</a>`;
	document.querySelector('.promosFilter .dropdown-content').innerHTML += promoHTML;
}
...
function addStarFilter(value) {
	...
	// Tạo text
	var text = `Trên ${value - 1} sao`;
	var starHTML = `<a href="${link}">${text}</a>`;

	document.querySelector('.starFilter .dropdown-content').innerHTML += starHTML;
}

// Thêm chọn sắp xếp
function addSortFilter(type, nameFilter, text) {
	var sortHTML = `<a href="javascript:void(0)" onclick="applySortFilter('${type}', '${nameFilter}')">${text}</a>`;
	document.querySelector('.sortFilter .dropdown-content').innerHTML += sortHTML;
}

// Hàm áp dụng sắp xếp
function applySortFilter(type, nameFilter) {
	// Lấy params hiện tại
	var currentParams = new URLSearchParams(window.location.search);

	// Reset phân trang khi đổi sắp xếp
	currentParams.delete('page');

	// Đặt sort theo format hệ thống đang dùng: by-type (vd: price-ascending)
	currentParams.set('sort', `${nameFilter}-${type}`);

	// Cập nhật URL không reload
	var newUrl = window.location.pathname + '?' + currentParams.toString();
	window.history.pushState({}, '', newUrl);

	// Load lại danh sách sản phẩm theo filter mới
	loadProducts();
}

// Tái render sản phẩm theo URL hiện tại (không reload trang)
function loadProducts() {
	// Xóa sản phẩm cũ
	clearAllProducts();

	// Xóa UI bộ lọc đã chọn và khởi tạo lại khung
	var choosed = document.getElementsByClassName('choosedFilter')[0];
	if (choosed) {
		choosed.innerHTML = '<a id="deleteAllFilter" style="display: none;"><h3>Xóa bộ lọc</h3></a>';
	}

	// Phân tích URL và áp dụng filter + sort
	var filters = getFilterFromURL();
	var sanPhamPhanTich = phanTich_URL(filters, true);

	// Tính toán phân trang và render
	...
	// Hiển thị khối sản phẩm (phòng trường hợp đang ẩn)
	var contain = document.getElementsByClassName('contain-products')[0];
	if (contain) contain.style.display = '';

	// Cập nhật các filter đã chọn
	addAllChoosedFilter();
}
