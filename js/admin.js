...
let charts = {}; // Lưu tất cả các biểu đồ theo id

function addChart(id, chartOption) {
    // Nếu biểu đồ cũ tồn tại thì xóa trước khi vẽ lại
    if (charts[id]) {
        charts[id].destroy();
    }
    let ctx = document.getElementById(id).getContext('2d');
    charts[id] = new Chart(ctx, chartOption);
}
...
function addThongKe() {
    ...
    danhSachDonHang.forEach(donHang => {
        ...
        donHang.sp.forEach(sanPhamTrongDonHang => {
            ...
            tongSoLuongBanRa += soLuong;
            tongDoanhThu += thanhTien;
        })
    })

    // Lấy mảng màu để vẽ đồ thị
   let colors = ['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0']; // 4 màu cố định
    ...
    // Hiển thị tổng số lượng và doanh thu
    document.getElementById('tongSoLuongBanRa').innerText = 'Tổng số lượng bán ra: ' + tongSoLuongBanRa;
    document.getElementById('tongDoanhThu').innerText = 'Tổng doanh thu: ' + numToString(tongDoanhThu);
}
...
function addKhungSuaSanPham(masp) {
    ...
    var company = ["Apple", "Samsung", "Oppo", "Huawei" ];
        ...
        <tr>
            ...
            <td>
                <select>
                    ...
                    <option value="moiramat" `+(sp.promo.name == 'moiramat'?'selected':'')+`>Sản phẩm hot</option>
                </select>
            </td>
        </tr>
        <tr>
...
function locDonHangTheoKhoangNgay() {
    // Read raw input values to control inclusive range
    var fromRaw = document.getElementById('fromDate').value;
    var toRaw = document.getElementById('toDate').value;

    // Build date range; if missing, use min/max
    var from = fromRaw ? new Date(fromRaw) : new Date(-8640000000000000); // earliest
    var to = toRaw ? new Date(toRaw) : new Date(8640000000000000); // latest

    // Make 'to' inclusive to the end of the selected day
    if (toRaw) to.setHours(23, 59, 59, 999);

    var listTr_table = document.getElementsByClassName('donhang')[0]
        .getElementsByClassName('table-content')[0]
        .getElementsByTagName('tr');

    for (var tr of listTr_table) {
        // Prefer the hidden machine-readable date in column 2 (order code = ngaymua)
        // Column indexes: [0] stt, [1] ma (ISO-like), [5] ngaygio (locale string)
        var isoCell = tr.getElementsByTagName('td')[1];
        var d = new Date(isoCell ? isoCell.textContent.trim() : '');

        // Fallback to the displayed locale date if needed
        if (isNaN(d.getTime())) {
            var localeCell = tr.getElementsByTagName('td')[5];
            d = new Date(localeCell ? localeCell.textContent.trim() : '');
        }
